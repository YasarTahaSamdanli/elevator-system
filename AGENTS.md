...
Asansör (elevator) maintenance and service management system for a single maintenance company (multi-tenant/SaaS is an explicit future possibility, not current scope — see `SOLUTION_ARCHITECTURE.md` section 14). Monorepo with `backend/` (Laravel 12, implemented), `web/` (React 18 + Vite + Tailwind/shadcn-style UI kit; all pages currently render mock data from `web/src/mock/` — not yet wired to the API) and `mobile/` (Flutter, technician-focused field app per ADR-003 — auth + work-order list/detail/status flows implemented against the real API; platform folders are not committed, run `flutter create --platforms=android,ios --org com.asansor --project-name asansor_mobile .` then `flutter pub get` inside `mobile/` first — see `mobile/README.md`). Web commands from `web/`: `npm run dev` (port 5173), `npm run build` (typecheck + build), `npm run lint` (tsc only).
...
- **Periodic inspections (etiket)**: `elevator_inspections` + `inspection_findings`, denormalized label cache on elevators (`current_label`, `follow_up_due`, …) refreshed from the latest inspection, red/yellow labels auto-suggest a +30/+60-day follow-up, `POST elevator-inspections/{uuid}/work-order` opens a repair work order from unresolved findings. Elevators resolve by exact `filter[qr_identifier]` for the mobile QR flow. See `docs/inspection-and-finance-plan.md`.
- **Customer ledger (cari)**: append-only `account_transactions` (no update/delete routes; corrections are reverse entries) + company-defined `payment_methods`. Cost vs sale price split (`work_order_items.unit_price` = cost snapshot, `sale_unit_price` = customer price). Completing a work order posts a `part_charge`/`revision_charge` in the same transaction as the stock issue; `ledger:accrue-maintenance` (scheduled monthly, idempotent per contract+month) posts maintenance fees; `GET account-transactions/summary` returns per-type totals and balance.
